...
local menu_elements = {
    main_toggle = checkbox:new(true, get_hash("EXP_TRACK_FINAL_V1")),
}
...
on_render(function()
    ...
    local current_exp = lp:get_current_experience()
    local needed = lp:get_experience_remaining_next_level()
    
    if not current_exp or current_exp == 0 then return end

    if not is_initialized then
        starting_exp = current_exp
        session_start_time = get_time_since_inject()
        is_initialized = true
    end

    local gained = current_exp - starting_exp
    ...
    pcall(function()
        graphics.text_3d("EXP: +" .. format_num(gained), 
            vec3:new(draw_x, draw_y, base_pos:z() + height_start), 
            font_size, color_green(255))
        
        graphics.text_3d("Next: " .. format_num(needed), 
